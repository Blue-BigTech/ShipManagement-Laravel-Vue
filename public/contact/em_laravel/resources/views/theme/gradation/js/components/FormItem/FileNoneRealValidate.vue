    ...
    >
      ...
      <template v-if="form_item.file_capacity_limit > 0">
        <span v-if="!confirmMode"
          >{{
            form_item.file_capacity_limit / 1024
          }}MB以下の制限があります。</span
        >
      </template>
      <template v-if="form_item.file_type">
        <span v-if="!confirmMode"
          >{{ form_item.file_type }}のファイル形式を選択</span
        >
      </template>
      <span v-if="confirmMode && onFileChange">
        {{ img_name }}
      </span>
      <img
        v-show="confirmMode && uploadedImage"
      ...
      <div v-if="fileExt && validationMode" class="validateError">
        有効なファイル形式ではありません。
      </div>
      <div v-if="fileSize && validationMode" class="validateError">
        ファイルの上限サイズを超えています。
      </div>
    </ValidationProvider>
    <ValidationProvider
