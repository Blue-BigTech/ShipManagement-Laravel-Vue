<template>
  <div>
    ...
    >
      ...
      <div v-if="failedRules.required && active" class="validateError">
        {{ form_item.required_error_msg }}
      </div>
      <div v-if="failedRules.InputRestriction && active" class="validateError">
...
export default {
  ...
  methods: {
    onInput: function () {
      if( this.active && this.InputRestriction || this.TextCountLimit || this.ProhibitedText ){
        return false;
      }
      this.active = false;
    },
    onFocus: function() {
      ...
      if( this.active && !telValue || telValue == undefined || telValue == "") {
        this.active = false;
        return;
      }
    },
