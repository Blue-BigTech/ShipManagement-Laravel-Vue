<template>
  <div>
    <ValidationProvider
      v-if="form_item.required == 1"
      :name="form_item.lang_title"
      :rules="{
        InputRestriction: InputRestriction,
        TextCountLimit: TextCountLimit,
        ProhibitedText: ProhibitedText,
        required: true,
      }"
      :bails="false"
      v-slot="{ validated,invalid, touched, failedRules }"
      slim
    >
      <input
        type="password"
        :id="form_item.html_id"
        :name="form_item.name"
        :placeholder="form_item.placeholder"
        :class="{
          invalid: invalid,
          touched: touched,
          validated:validated,
          confirmMode: confirmMode,
          InputRestriction: InputRestriction,
        }"
        class="form__input"
        v-model="passwordValue"
      />
      <span v-if="confirmMode">
        {{ passwordValue }}
      </span>
      <div v-if="failedRules.required" class="validateError">
        {{ form_item.required_error_msg }}
      </div>
      <div v-if="failedRules.InputRestriction" class="validateError">
        {{ form_item.restriction_error_msg }}
      </div>
      <div v-if="failedRules.TextCountLimit" class="validateError">
        {{ form_item.length_error_msg }}
      </div>
      <div v-if="failedRules.ProhibitedText" class="validateError">
        {{ form_item.forbidden_characters }}は入力できません。
      </div>
    </ValidationProvider>
    <ValidationProvider
      v-if="form_item.required == 0"
      :name="form_item.lang_title"
      :rules="{
        InputRestriction: InputRestriction,
        TextCountLimit: TextCountLimit,
        ProhibitedText: ProhibitedText,
      }"
      :bails="false"
      v-slot="{ invalid, failedRules }"
      slim
    >
      <input
        type="password"
        @input="onInput"
        @blur="onBlur"
        @focus="onFocus"
        :id="form_item.html_id"
        :name="form_item.name"
        :placeholder="form_item.placeholder"
        :class="{
          invalid: invalid && active,
          confirmMode: confirmMode,
          InputRestriction: InputRestriction,
          ProhibitedText: ProhibitedText,
        }"
        class="form__input"
        v-model="passwordValue"
      />
      <span v-if="confirmMode">
        {{ passwordValue }}
      </span>
      <div
        v-if="failedRules.InputRestriction && active"
        class="validateError"
      >
        {{ form_item.restriction_error_msg }}
      </div>
      <div
        v-if="failedRules.TextCountLimit && active"
        class="validateError"
      >
        {{ form_item.length_error_msg }}
      </div>
      <div
        v-if="failedRules.ProhibitedText && active"
        class="validateError"
      >
        {{ form_item.forbidden_characters }}は入力できません。
      </div>
    </ValidationProvider>
  </div>
</template>

<script>
import Vue from "vue";
import { ValidationProvider, localize, extend } from "vee-validate";
import { required } from "vee-validate/dist/rules";

extend("required", required);

//入力制限
extend("InputRestriction", {
  // エラーメッセージを設定する
  validate(value) {
    return false;
  },
});
//入力文字数
extend("TextCountLimit", {
  // エラーメッセージを設定する
  validate(value) {
    return false;
  },
});
//禁止文字
extend("ProhibitedText", {
  // エラーメッセージを設定する
  validate(value) {
    return false;
  },
});

// 日本語化
import ja from "vee-validate/dist/locale/ja";
localize("ja", ja);

// コンポーネント設定
Vue.component("ValidationProvider", ValidationProvider);

export default {
  props: {
    form_item: {
      type: Object,
    },
    confirmMode: {
      type: Boolean,
    },
  },
  data() {
    return {
      passwordValue: "",
      active: false,
    };
  },
  watch: {
    passwordValue: function(str) {
      const form_item = this.form_item;
      if(form_item.hankaku_zenkaku_automatic_conversion == "default"){
        return false;
      }
      if( form_item.hankaku_zenkaku_automatic_conversion == "hankaku") {
        this.passwordValue = this.toHankaku(str);
      }
      if( form_item.hankaku_zenkaku_automatic_conversion == "zenkaku") {
        this.passwordValue = this.toZenkaku(str);
      }
    },
  },
  methods: {
    onInput: function () {
      if (
        (this.active && this.InputRestriction) ||
        this.TextCountLimit ||
        this.ProhibitedText
      ) {
        return false;
      }
      this.active = false;
    },
    onFocus: function () {
      const textValue = this.textValue;
      if (
        (this.active && !textValue) ||
        textValue == undefined ||
        textValue == ""
      ) {
        this.active = false;
        return;
      }
    },
    onBlur: function () {
      this.active = true;
    },
  },
  computed: {
    InputRestriction: function () {
      const form_item = this.form_item;
      const passwordValue = this.passwordValue;
      if (!form_item.restriction) {
        return false;
      }
      //ひらがな
      if (form_item.restriction == "hiragana") {
        if (
          !passwordValue ||
          passwordValue == undefined ||
          passwordValue == ""
        ) {
          return false;
        }
        if (passwordValue.match(/^[ぁ-んー　]+$/)) {
          return false;
        } else {
          return true;
        }
      }
      //カタカナ
      if (form_item.restriction == "katakana") {
        if (
          !passwordValue ||
          passwordValue == undefined ||
          passwordValue == ""
        ) {
          return false;
        }
        if (passwordValue.match(/^[ァ-ヶー　]*$/)) {
          //"ー"の後ろの文字は全角スペースです。
          return false;
        } else {
          return true;
        }
      }
      //半角数字
      if (form_item.restriction == "num") {
        if (
          !passwordValue ||
          passwordValue == undefined ||
          passwordValue == ""
        ) {
          return false;
        }
        if (passwordValue.match(/^[0-9]*$/)) {
          return false;
        } else {
          return true;
        }
      }
      //半角英数字
      if (form_item.restriction == "alphabet_num") {
        if (
          !passwordValue ||
          passwordValue == undefined ||
          passwordValue == ""
        ) {
          return false;
        }
        if (passwordValue.match(/^[0-9a-zA-Z]*$/)) {
          return false;
        } else {
          return true;
        }
      }
      //半角英字
      if (form_item.restriction == "alphabet") {
        if (
          !passwordValue ||
          passwordValue == undefined ||
          passwordValue == ""
        ) {
          return false;
        }
        if (passwordValue.match(/^[a-zA-Z]*$/)) {
          return false;
        } else {
          return true;
        }
      }
      //半角英数字混在
      if (form_item.restriction == "alphabet_num_mix") {
        if (
          !passwordValue ||
          passwordValue == undefined ||
          passwordValue == ""
        ) {
          return false;
        }
        if (passwordValue.match(/([0-9].*[a-zA-Z]|[a-zA-Z].*[0-9])/s)) {
          return false;
        }
        let num_flag = false;
        if (passwordValue.match(/^[0-9]*$/)) {
          num_flag = true;
        }
        let alphabet_flag = false;
        if (passwordValue.match(/^[a-zA-Z]*$/)) {
          alphabet_flag = true;
        }
        if (num_flag == true && alphabet_flag == true) {
          return true;
        } else {
          return true;
        }
      }
      //半角数字と半角ハイフン
      if (form_item.restriction == "num_hyphen") {
        if (
          !passwordValue ||
          passwordValue == undefined ||
          passwordValue == ""
        ) {
          return false;
        }
        if (passwordValue.match(/^[0-9\-]+$/)) {
          return false;
        } else {
          return true;
        }
      }
      //メールアドレス
      if (form_item.restriction == "email") {
        if (
          !passwordValue ||
          passwordValue == undefined ||
          passwordValue == ""
        ) {
          return false;
        }
        if (
          passwordValue.match(
            /^([A-Za-z0-9!"#$%&'()*+,-./:;<=>?@[\]^_`{|}])+([A-Za-z0-9!"#$%&'()*+,-./:;<=>?@[\]^_`{|}\._-])*@([A-Za-z0-9!"#$%&'()*+,-./:;<=>?@[\]^_`{|}_-])+([A-Za-z0-9!"#$%&'()*+,-./:;<=>?@[\]^_`{|}\._-]+)+$/
          )
        ) {
          return false;
        } else {
          return true;
        }
      }
      //電話番号09000000000
      if (form_item.restriction == "tel") {
        if (
          !passwordValue ||
          passwordValue == undefined ||
          passwordValue == ""
        ) {
          return false;
        }
        if (passwordValue.match(/^\d{7,13}$/)) {
          return false;
        } else {
          return true;
        }
      }
      //電話番号090-0000-0000
      if (form_item.restriction == "tel_hyphen") {
        if (
          !passwordValue ||
          passwordValue == undefined ||
          passwordValue == ""
        ) {
          return false;
        }
        if (passwordValue.match(/^(\d{2,3}-){0,1}\d{1,4}-\d{2,4}-\d{2,4}$/)) {
          return false;
        } else {
          return true;
        }
      }
      //郵便番号1001111
      if (form_item.restriction == "zip") {
        if (
          !passwordValue ||
          passwordValue == undefined ||
          passwordValue == ""
        ) {
          return false;
        }
        if (passwordValue.match(/^\d{7}$/)) {
          return false;
        } else {
          return true;
        }
      }
      //郵便番号100-1111
      if (form_item.restriction == "zip_hyphen") {
        if (
          !passwordValue ||
          passwordValue == undefined ||
          passwordValue == ""
        ) {
          return false;
        }
        if (passwordValue.match(/^\d{3}$|^\d{3}-\d{2}$|^\d{3}-\d{4}$/)) {
          return false;
        } else {
          return true;
        }
      }
    },
    TextCountLimit: function () {
      const form_item = this.form_item;
      const passwordValue = this.passwordValue;
      if (!passwordValue || passwordValue == undefined || passwordValue == "") {
        return false;
      }
      if (form_item.min_length > 0 && form_item.max_length > 0) {
        if (
          passwordValue.length < form_item.min_length ||
          passwordValue.length > form_item.max_length
        ) {
          return true;
        } else {
          return false;
        }
      }
      if (form_item.min_length > 0) {
        if (passwordValue.length < form_item.min_length) {
          return true;
        } else {
          return false;
        }
      }
      if (form_item.max_length > 0) {
        if (passwordValue.length > form_item.max_length) {
          return true;
        } else {
          return false;
        }
      }
      return false;
    },
    ProhibitedText: function () {
      const form_item = this.form_item;
      const passwordValue = this.passwordValue;
      if (!passwordValue || passwordValue == undefined || passwordValue == "") {
        return false;
      }
      if (form_item.forbidden_characters) {
        if (passwordValue === form_item.forbidden_characters) {
          return true;
        } else {
          return false;
        }
      }
      return false;
    },
  },
};
</script>
