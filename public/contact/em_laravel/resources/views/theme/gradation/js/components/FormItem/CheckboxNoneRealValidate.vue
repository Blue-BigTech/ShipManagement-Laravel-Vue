<template>
  <div>
    <ValidationProvider
      ...
      :rules="{ required: { allowFalse: false } ,checkboxValidate: checkboxValidate}"
      :bails="false"
      v-slot="{ failedRules }"
      slim
    >
      ...
      >
        <template
          v-for="value in checkboxValue"
        >
          {{ value }}
        </template>
      </span>
      <div v-if="validationMode && checkboxValidate || validationMode && failedRules.required" class="validateError">{{ form_item.required_error_msg }}</div>
    </ValidationProvider>
